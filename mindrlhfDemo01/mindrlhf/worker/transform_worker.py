...
class TransformWorker(Worker):
    ...
    def __init__(
        ...
        with TimeConsumingCollector("TransformWorker init"):
            if get_rank() == 0:
                ...
                if grpo_config.rl_config.enable_ref:
                    ms.merge_pipeline_strategys(
                        os.path.join(self.save_strategy_dir, "infer_ref_strategy"), ref_merged_stra)
    ...
    def reshard_params(self, step_num, input_on_device_flag_dict=None):
        ...
        with TimeConsumingCollector("reshard params"):
            ...
            if (
                self.sync_ref_model
                and (step_num + 1) % self.ref_model_sync_steps == 0
                and self.grpo_config.rl_config.enable_ref
            ):
